...
def _collapse_sources(source_rows: pd.DataFrame) -> pd.DataFrame:
    ...
    grouped = source_rows.groupby(["player_name", "org", "level"], dropna=False)

    for (player_name, org, level), group in grouped:
        source_rank_map = {
            str(source): rank
        ...
        grouped_records.append(
            {
                ...
                "org": org,
                "level": level,
                "positions": _first_non_null(group["positions"]),
                "age": _first_non_null(group["age"]),
                "eta": _first_non_null(group["eta"]),
                "bats": _first_non_null(group["bats"]),
                "throws": _first_non_null(group["throws"]),
                "fv": _first_non_null(group["fv"]),
                "ofp": _first_non_null(group["ofp"]),
                "stats_summary": _first_non_null(group["stats_summary"]),
                "scouting_report": _first_non_null(group["scouting_report"]),
                "notes": _first_non_null(group["notes"]),
                "payload_scraped_at": _first_non_null(group["payload_scraped_at"]),
                "collected_at": _first_non_null(group["collected_at"]),
                "source_url": _first_non_null(group["source_url"]),
...
def build_prospects_snapshot(
    ...
    if stats_rows is not None and not stats_rows.empty:
        ...
        available_columns = [c for c in stats_columns if c in stats_rows.columns]
